...
export default {
  setup() {
    ...
    const fetchCsrfTokenAndUserConfig = async () => {
      try {
        const csrfResponse = await fetch("https://localhost:3002/csrf-token", {
          method: "GET",
          credentials: "include",
        });
        ...
        const configResponse = await fetch(
            `https://localhost:3002/getUserConfig`,
            {
              method: "GET",
              headers: {
    ...
    const stopTracking = async (skin) => {
      try {
        ...
        const deleteResponse = await fetch("https://localhost:3002/deleteSkin", {
          method: "DELETE",
          headers: {
